...
#--------------------------------------------------------------
# read in test file to ary2

ary2 = np.zeros((countmax,23))
...
for line in fin:
  if ('lr' in line):
    ...
    for k in range(0,12):
      ary2[count,k] = float(words[k+3])

    for l in range(0,6):
      ary2[count,12+l] = (ary2[count,2*l] - ary2[count,2*l+1])/(ary2[count,2*l] + ary2[count,2*l+1])
      pr[count,l] = (ary2[count,2*l] - ary2[count,2*l+1])/(ary2[count,2*l] + ary2[count,2*l+1])

    ary2[count,18] = flag[tj, ti]
    ary2[count,19] = svals[tj, ti]
    ary2[count,20] = avals[tj, ti]
    ary2[count,21] = tlat
    ary2[count,22] = tlon
...
# use hard clustering since points are (in training) never questionable (p > .99)
yyy = cluster.predict(ary2[:count,:18] )
ccount = np.zeros((nlim))
acount = np.zeros((nlim))
for i in range(0, count):
  ccount [ yyy[i] ] += 1
