...
#               firstlon = firstlon, firstlat = firstlat)
  def  __init__(self, z, line):
    ...
    #retain the initial location for reference -- 
    #    self.lat, self.lon will become .nc variables
    ...
    if (self.i < 0 or self.j < 0):
      print("invlocate issue ",self.lat, self.lon, z.firstlat, z.firstlon, 
             z.dlat, z.dlon, flush=True)
      exit(1)
...
#no effect -- superceded by way vars are added per time level
  ...
  def pncopen3(self, fname, tag, cyc):
    #RG should be able to call regular and add the level stuff here
    self.ncfile = nc.Dataset(fname, mode='w', format='NETCDF4')
    self.lat_dim = self.ncfile.createDimension('lat', 2*self.range_x+1)
    self.lon_dim = self.ncfile.createDimension('lon', 2*self.range_y+1)
    self.time_dim = self.ncfile.createDimension('time', None)
    self.level_dim = self.ncfile.createDimension('level', None)
    # Create variables to hold values for those referenced dimensions
    self.lat = self.ncfile.createVariable('lat', np.float32, ('lat',))
    self.lat.units = 'degrees_north'
    self.lat.long_name = 'latitude'
    ...
    self.level = self.ncfile.createVariable('level',np.float32, ('level',))
    self.level.units = 'hPa'
##RG Trial
#    self.time[:] = np.arange(0,121,6)
#no effect -- superceded by way vars are added per time level
...
# RG: need to manage name repeats.
  ...
  def extractvar3(self, ftime, allvalues, vname, level):
    ...
    if (self.i*self.j != 0) :
      ...
      #       self.range_y, "level: ",level, flush=True)
   
      #manage vname, time slicing:
      self.ncfile.variables[vname][ftime,level,:,:] = y
