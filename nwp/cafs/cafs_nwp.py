...
for j in range(0,ny):
  ...
  for i in range(0,nx):
    ...
    if (im >= 0):
      if (nodemap[j,im] != 0):
        weight = cost(cost_type, lat1 = lats[j,i], lon1 = lons[j,i], lat2 = lats[j,im], lon2 = lons[j,im])
        G.add_edge(n, nodemap[j,im], weight= weight)
        k += 1

    if (ip < nx):
      if (nodemap[j,ip] != 0):
        weight = cost(cost_type, lat1 = lats[j,i], lon1 = lons[j,i], lat2 = lats[j,ip], lon2 = lons[j,ip])
        G.add_edge(n, nodemap[j,ip], weight = weight)
        k += 1

    if (jp < ny ):
      if (nodemap[jp,i] != 0):
        weight = cost(cost_type, lat1 = lats[j,i], lon1 = lons[j,i], lat2 = lats[jp,i], lon2 = lons[jp,i])
        G.add_edge(n, nodemap[jp,i], weight = weight)
        k += 1
      if (im >= 0):
        if (nodemap[jp,im] != 0):
          weight = cost(cost_type, lat1 = lats[j,i], lon1 = lons[j,i], lat2 = lats[jp,im], lon2 = lons[jp,im])
          G.add_edge(n, nodemap[jp,im], weight = weight)
          k += 1
      if (ip < nx):
        if (nodemap[jp,ip] != 0):
          weight = cost(cost_type, lat1 = lats[j,i], lon1 = lons[j,i], lat2 = lats[jp,ip], lon2 = lons[jp,ip])
          G.add_edge(n, nodemap[jp,ip], weight = weight)
          k += 1

    if (jm >= 0 ):
      if (nodemap[jm,i] != 0):
        weight = cost(cost_type, lat1 = lats[j,i], lon1 = lons[j,i], lat2 = lats[jm,i], lon2 = lons[jm,i])
        G.add_edge(n, nodemap[jm,i], weight = weight)
        k += 1
      if (im >= 0):
        if (nodemap[jm,im] != 0):
          weight = cost(cost_type, lat1 = lats[j,i], lon1 = lons[j,i], lat2 = lats[jm,im], lon2 = lons[jm,im])
          G.add_edge(n, nodemap[jm,im], weight = weight)
          k += 1
      if (ip < nx):
        if (nodemap[jm,ip] != 0):
          weight = cost(cost_type, lat1 = lats[j,i], lon1 = lons[j,i], lat2 = lats[jm,ip], lon2 = lons[jm,ip])
