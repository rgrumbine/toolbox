...
def show( tlats, tlons, tag, hours = 6, cost = 3482, reference = 3482, close = True):
...
#          (0.125, 0.10),  xycoords = 'figure fraction', size = 16 )
  ...
  plt.annotate("Cost is defined as 1.1/(1.1-Ai) – 1 at zero ice concentration.",(0.05, 0.835),
                  xycoords = 'figure fraction', size = 16 )
  plt.annotate("For a completely ice-free route, the minimum penalty is 3482.",(0.05, 0.805),
                  xycoords = 'figure fraction', size = 16 )
  plt.annotate("Any cost greater than this indicates at least some ice.",(0.05, 0.775),
                  xycoords = 'figure fraction', size = 16 )
  plt.annotate("Costs can be over 10K in winter.",(0.05, 0.745),
                  xycoords = 'figure fraction', size = 16 )
  plt.annotate("These northwest passage maps are suggestive, not actual guidance.",(0.05, 0.22),
                  xycoords = 'figure fraction', size = 16 )
  plt.annotate("The passage displayed shows the least expensive path.",(0.05, 0.19),
                  xycoords = 'figure fraction', size = 16 )
  plt.annotate("At this time, polar class of the ship is not considered.",(0.05, 0.16),
                  xycoords = 'figure fraction', size = 16 )
  plt.annotate("Output fields from this diagnostic (including kml files) available by request.",(0.05, 0.13),
                  xycoords = 'figure fraction', size = 16 )
  plt.annotate("Suggestions for improving the graphics are greatly welcome!",(0.05, 0.1),
                  xycoords = 'figure fraction', size = 16 )
  plt.annotate("Please send display and support file suggestions to Robert.Grumbine@noaa.gov",(0.05, 0.07),
                  xycoords = 'figure fraction', size = 16 )
...
def overlay(proj, ax, fig, tlats, tlons, color = "blue"):
  ''' overlay -- to overlay lines, cannot close the plt '''
  plt.scatter(tlons, tlats, transform = ccrs.PlateCarree(),
