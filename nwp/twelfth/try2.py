...
for j in range(0,ny):
  ...
  for i in range(0,nx):
    ...
    if (jp < ny ):
      ...
      if (im >= 0):
        if (nodemap[jp,im] != 0):
          G.add_edge(n, nodemap[jp,im])
          k += 1
      if (ip < nx):
        if (nodemap[jp,ip] != 0):
          G.add_edge(n, nodemap[jp,ip])
          k += 1
    ...
    if (jm >= 0 ):
      ...
      if (im >= 0):
        if (nodemap[jm,im] != 0):
          G.add_edge(n, nodemap[jm,im])
          k += 1
      if (ip < nx):
        if (nodemap[jm,ip] != 0):
          G.add_edge(n, nodemap[jm,ip])
          k += 1
...
#print("finish -- full ",int(0.5+(360.-78.0)*12.), int(0.5 + (90.0 - 74.0)*12.) )
#i_finish = 2297+int((3384-2297)/3)
#j_finish = 292+int((192-292)/3)
