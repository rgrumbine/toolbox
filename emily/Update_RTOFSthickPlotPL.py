...
#for k in fruit: #or k range(grb):
    plt.figure()
    RBER_Name= str("rtofs_BER_thick_")+str(i) +str("_")+datename+str(".grib2")
    RBER_Name= str("rtofs_BER_thick_")+str(i) +str("_")+datename+str(".grib2")
    RARC_Name= str("rtofs_ARC_thick_")+str(i) +str("_")+datename+str(".grib2")
    gribB = RBER_Name
    gribA = RARC_Name
    ...
    for g in grbsA:
        print(g.name)
    print(grbsA)
    grbA = grbsA.select(name='Sea-ice thickness')[0]
    grbB = grbsB.select(name='Sea-ice thickness')[0]
    dataA=(grbA.values)
    dataB=(grbB.values)
   ...
   # print(data)
    ...
    bounds = [  0.25, 0.50 , 0.75, 1.0,1.25,1.50, 1.75, 2.0]
...
#plt.savefig('GOFS_goDay3.png', bbox_inches="tight")
    figName100DU = "RTOFS_th" + str(i) +".png"  
 #   pupppy=   plt.contour(x,y,dataB, levels = range(0,50,5), colors='k',interpolation='none')
 #   plt.clabel(pupppy)  ticks=bounds, 

    plt.clim(0.0, 2.0);
    plt.colorbar(cs,orientation='vertical', shrink=0.5, spacing='proportional',  cmap=cmap)
