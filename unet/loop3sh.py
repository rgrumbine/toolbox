...
for yy in range(starting, starting+int(ndata/12)):
  for mm in range(1,13):
    ...
    analy.close()
    deflag(tmp2)
    #RG: have to transfer the nsidc grid in to augmented (336x320 vs. 332x316) grid
    X_data[count,:ny_nsidc,:nx_nsidc,0] = tmp2
    ...
    X_data[count,:,:,5] = cos(3*2.*pi*(mm+nlag-1)/12.)
    X_data[count,:,:,6] = sin(3*2.*pi*(mm+nlag-1)/12.)
...
analy.close()
deflag(tmp2)
y_data[count-1,:ny_nsidc,:nx_nsidc,0] = tmp2
...
print(f"Memory usage after readin: {current / 10**6} Mb")
...
# Get memory metrics: (current, peak)
current, peak = tracemalloc.get_traced_memory()
print(f"Memory usage after model summary: {current / 10**6} Mb")
print(f"Peak memory usage: {peak / 10**6} Mb", flush=True)
...
for period in range(0,20):
  ...
  # Get memory metrics: (current, peak)
  current, peak = tracemalloc.get_traced_memory()
  print(f"Memory usage after period {period:02d} training: {current / 10**6} Mb")
  print(f"Peak memory usage: {peak / 10**6} Mb", flush=True)
  ...
  plt.tight_layout()
  plt.savefig(f'shperiod{period:d}.png')
