...
def deflag(tmp):
    tmp[tmp > 100] = 0
# ---------------------------------------------------------
# Input (X): Sea ice concentration at time (t-1), cos(month), sin(month)
...
for yy in range(starting, starting+int(ndata/12)):
  for mm in range(1,13):
    ...
    deflag(tmp2)
    X_data[count,:,:,0] = tmp2
    X_data[count,:,:,1] = cos(2.*pi*(mm-1)/12.)
    X_data[count,:,:,2] = sin(2.*pi*(mm-1)/12.)
    X_data[count,:,:,3] = cos(2*2.*pi*(mm-1)/12.)
    X_data[count,:,:,4] = sin(2*2.*pi*(mm-1)/12.)
    analy.close()
    count += 1
# Scale ice concentrations, sin and cos are already scaled
m = X_data[:,:,:,0].max()
X_data[:,:,:,0] /= m

# now assign next month (of X_data) to y_data
for i in range(0, count-1):
    y_data[i,:,:,0] = X_data[i+1,:,:,0]
...
# Save the model:
import joblib
joblib.dump(unet_model, "unet_model.joblib")
