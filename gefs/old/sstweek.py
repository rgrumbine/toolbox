...
if (os.path.exists('sstweek.joblib')):
  unet = joblib.load('sstweek.joblib')
else:
  print("building the unet model", flush=True)
  unet = build_unet(input_shape=(ny,nx,nlayer), final='linear')
...
# save the unet
joblib.dump(unet, "sstweek.joblib")
# Get memory metrics: (current, peak)
current, peak = tracemalloc.get_traced_memory()
...
#--------------------------------------------------------------------------------
# Make a sample result plot
show(unet, Xval, yval, figname = "sstsample.png")

# Get memory metrics: (current, peak)
current, peak = tracemalloc.get_traced_memory()
print(f"past graphics memory usage: {current / 10**6} Mb")
print(f"Peak memory usage: {peak / 10**6} Mb", flush=True)
