...
end   = datetime.datetime(1988,4,20)
...
while(tag <= end ):
  ...
  flx = nc.Dataset('week.'+tag.strftime("%Y%m%d")+'.nc')
...
if (os.path.exists('weekice.joblib')):
  begin = time.time()
  print("about to load joblib",flush=True)
  unet = joblib.load('weekice.joblib')
...
# save the unet
joblib.dump(unet, "weekice.joblib")
