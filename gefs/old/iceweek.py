...
if (os.path.exists('iceweek.joblib')):
  print("about to load joblib",flush=True)
  unet = joblib.load('iceweek.joblib')
else:
  print("building the unet model", flush=True)
  unet = build_unet(input_shape=(ny,nx,nlayer), final='sigmoid')
...
# save the unet
joblib.dump(unet, "iceweek.joblib")
# Get memory metrics: (current, peak)
current, peak = tracemalloc.get_traced_memory()
...
#--------------------------------------------------------------------------------
# Visualize:
show(unet, Xval, yval, figname="icesample.png")
