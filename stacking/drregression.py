...
#xtrain[:,2] = (xtrain[:,0]-xtrain[:,1])/(xtrain[:,0]+xtrain[:,1])
#xtrain[:,3] = (xtrain[:,3]-xtrain[:,4])/(xtrain[:,3]+xtrain[:,4])
#xtrain[:,2] = (xtrain[:,3]-xtrain[:,2])/(xtrain[:,3]+xtrain[:,2])

ytrain[:] = data.variables['ice_concentration'][:]
ytrain *= 100
#debug: print('ytrain max ',i, ytrain[:].max(), ytrain[:].min(), flush=True )
